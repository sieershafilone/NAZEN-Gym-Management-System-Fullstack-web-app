        ...
        <div className="space-y-16 animate-fade-in pb-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-12">
                <div className="space-y-4">
                    <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.5em]">Gallery</p>
                    <h1 className="text-6xl font-black text-white tracking-tighter uppercase leading-none">
                        GYM <span className="text-cyan-400">GALLERY</span>
                    </h1>
                    <p className="text-zinc-500 font-bold uppercase text-[9px] tracking-widest mt-6">Checkout our members and facilities</p>
                </div>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                    <div key={i} className="aspect-[4/5] bg-white/[0.03] backdrop-blur-xl rounded-[2rem] border border-white/10 flex items-center justify-center group overflow-hidden relative cursor-pointer transition-all duration-700 hover:border-cyan-400/30">
                        ...
                        <div className="absolute bottom-6 left-6 right-6 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 z-20">
                            <p className="text-[8px] font-black text-cyan-400 uppercase tracking-widest mb-1">PHOTO 00{i}</p>
            ...
            <div className="text-center py-24 bg-white/[0.01] backdrop-blur-2xl rounded-[3rem] border border-white/5 border-dashed">
                <p className="text-zinc-800 font-black uppercase text-[10px] tracking-[0.6em]">More photos coming soon...</p>
            </div>
        </div>
    );
}
