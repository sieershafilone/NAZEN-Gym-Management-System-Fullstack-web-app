            ...
            <div className="flex flex-col items-center justify-center p-12 text-center h-[70vh] relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-8 max-w-lg">
                    ...
                    <div className="space-y-4">
                        <h2 className="text-4xl font-black text-white tracking-tighter uppercase">No Active <span className="text-rose-500">Authorization</span></h2>
                        <p className="text-zinc-600 font-bold uppercase text-[9px] tracking-[0.2em] leading-relaxed">
                            Your access privileges have been <span className="text-white">Suspended</span> or are not yet active. <br />
                            Consult the <span className="text-white">Logistics Command</span> for activation.
                        </p>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="space-y-16 animate-fade-in pb-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-12">
                <div className="space-y-4">
                    <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.5em]">Credentials Hub</p>
                    <h1 className="text-6xl font-black text-white tracking-tighter uppercase leading-none">
                        MY <span className="text-cyan-400">MEMBERSHIP</span>
                    </h1>
                    <p className="text-zinc-500 font-bold uppercase text-[9px] tracking-widest mt-6">Protocol synchronization & flux history</p>
                </div>
            </div>

            {/* Matrix Status Segment */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <Card variant="default" className="p-0 overflow-hidden relative group rounded-[3rem] border-white/5 bg-white/[0.01] hover:bg-white/[0.03] transition-all duration-700">
                    ...
                    <div className="p-10 relative z-10">
                        <div className="flex justify-between items-start mb-12">
                            <div className="space-y-2">
                                <h3 className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.4em]">Current Protocol</h3>
                        ...
                        <div className="space-y-10 mb-12">
                            <div className="flex items-center gap-6 bg-[#0D0D0D] p-6 rounded-[2rem] border border-white/5">
                                ...
                                <div>
                                    <p className="text-[8px] font-black text-zinc-600 uppercase tracking-widest">Persistence Data</p>
                                    <p className="text-sm font-black text-white uppercase tracking-widest mt-1">CALIBRATED UNTIL {formatDate(membership.endDate)}</p>
                                </div>
                            </div>

                            <div className="bg-[#0D0D0D] rounded-[2.5rem] p-8 border border-white/5 transition-all group/progress">
                                <div className="flex justify-between items-end mb-6">
                                    <span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Flux Duration Remaining</span>
                ...
                {/* Entry Log History */}
                <Card variant="default" className="rounded-[3rem] p-10 bg-[#080808] border-white/5 overflow-hidden flex flex-col">
                    <div className="flex items-center justify-between mb-12">
                        <div className="space-y-1">
                            ...
                            <p className="text-[9px] font-black text-zinc-600 uppercase tracking-[0.4em] pl-16">Currency injection history</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto pr-4 scrollbar-hide">
                        {loading ? (
                            ...
                            <div className="py-20 text-center bg-[#0D0D0D] rounded-3xl border border-white/5 border-dashed">
                                <p className="text-zinc-600 font-black uppercase text-[10px] tracking-widest">No flux recorded</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {payments.map((payment) => (
                                    <div key={payment.id} className="flex items-center justify-between p-6 bg-[#0D0D0D] border border-white/5 rounded-[2rem] group/log hover:border-cyan-400/20 transition-all">
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div>
                                                <p className="text-[11px] font-black text-white uppercase tracking-widest">{payment.membership?.plan?.name || 'GENERIC INJECTION'}</p>
